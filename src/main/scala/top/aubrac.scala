 ...
 * -----                                                                       *
 * Last Modified: 2023-02-27 05:39:25 pm                                       *
...
class Aubrac (p: AubracParams) extends Module {
  ...
  if (p.useChamp) m_l0dcross.io.b_m(1) <> m_dmu.get.io.b_dmem
  ...
  if (p.useChamp) {
    ...
    for (d <- 0 until p.nDome) {
      ...
      m_dmu.get.io.b_dome(d).free := w_dome_free(d).asUInt.andR
    }
    ...
    for (pa <- 0 until p.nPart) {
      ...
      m_dmu.get.io.b_pall.state(pa).free := w_pall_free(pa).asUInt.andR
    }
  }
